...
const products = async (req, res) => {
    authMiddleware(req,res, async () => {
        try{
            ...
            const extra_facets = ((lang==="en") ? process.env.Facets_English_Fields : 
            process.env.Facets_Spanish_Fields).split('__'); 
            ...
            if(req.body.RegionProductCategoryLocal){
                ...
                filterData += `RegionProductCategoryLocal${lang==="en" ? '': "_esMX"} eq '${req.body.RegionProductCategoryLocal}' `;
            ...
            if(req.body.TouchlessFaucet){
                filterData += (filterData != '') ? "and ": "";
                filterData += `TouchlessFaucet${lang==="en" ? '': "_esMX"} eq '${req.body.TouchlessFaucet}'`;
            }

            if(req.body.BowlShape){
                filterData += (filterData != '') ? "and ": "";
                filterData += `BowlShape${lang==="en" ? '': "_esMX"} eq '${req.body.BowlShape}'`;
            }

            if(req.body.HandleStyle){
                filterData += (filterData != '') ? "and ": "";
                filterData += `HandleStyle${lang==="en" ? '': "_esMX"} eq '${req.body.HandleStyle}'`;
            }

            if(req.body.BidetFunctionality){
                filterData += (filterData != '') ? "and ": "";
                filterData += `BidetFunctionality${lang==="en" ? '': "_esMX"} eq '${req.body.BidetFunctionality}'`;
            }

            if(req.body.ProductInstallationType){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `ProductInstallationType${lang==="en" ? '': "_esMX"} eq '${req.body.ProductInstallationType}'`;
            }

            if(req.body.ProductProductType){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `ProductProductType${lang==="en" ? '': "_esMX"} eq '${req.body.ProductProductType}'`;
            }
            ...
            if(req.body.ProductMaterial){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `ProductMaterial${lang==="en" ? '': "_esMX"} eq '${req.body.ProductMaterial}'`;
            }

            if(req.body.ProductShape){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `ProductShape${lang==="en" ? '': "_esMX"} eq '${req.body.ProductShape}'`;
            }
            ...
            if(req.body.SprayType){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `SprayType${lang==="en" ? '': "_esMX"} eq '${req.body.SprayType}'`;
            }
            ...
            if(req.body.ProductConfiguration){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `ProductConfiguration${lang==="en" ? '': "_esMX"} eq '${req.body.ProductConfiguration}'`;
            }
            ...
            if(req.body.NightLight){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `NightLight${lang==="en" ? '': "_esMX"} eq '${req.body.NightLight}'`;
            }
            ...
            if(req.body.toilet_type){
                ...
                filterData += `ProductInstallationType${lang==="en" ? '': "_esMX"} eq '${req.body.toilet_type}' or IntelligentToilet${lang==="en" ? '': "_esMX"} eq '${req.body.toilet_type}' or ProductConfiguration${lang==="en" ? '': "_esMX"} eq '${req.body.toilet_type}' `;
            ...
            if(req.body.NumberOfHoles){
                filterData += (filterData != '') ? "and ": ""; 
                filterData += `NumberOfHoles${lang==="en" ? '': "_esMX"} eq '${req.body.NumberOfHoles}'`;
            }
            ...
            if(response.value && response.value.length){
                ...
                if(req.body.search){
                    ...
                    const {result, specificationCount} = await getSpecificationCount(bodyData,lang);
                ...
                if( response["@search.facets"] && response["@search.facets"]["IntelligentToilet"]?.length){
    ...
    for (const iterator of products) {
        if(iterator.DWGPlanView || iterator.ThreeDDXF){
            ...
            if(SpecRegionProductCategoryLocal[loaclCat]){ 
                SpecRegionProductCategoryLocal[loaclCat] = SpecRegionProductCategoryLocal[loaclCat] + 1;
            }else{
...
export async function conversionKeys(response){
    ...
    response["@search.facets"]["TouchlessFaucet"] = response["@search.facets"]["TouchlessFaucet_esMX"];
    delete response["@search.facets"]["TouchlessFaucet_esMX"];

    response["@search.facets"]["BowlShape"] = response["@search.facets"]["BowlShape_esMX"];
    delete response["@search.facets"]["BowlShape_esMX"];

    response["@search.facets"]["HandleStyle"] = response["@search.facets"]["HandleStyle_esMX"];
    delete response["@search.facets"]["HandleStyle_esMX"];

    response["@search.facets"]["BidetFunctionality"] = response["@search.facets"]["BidetFunctionality_esMX"];
    delete response["@search.facets"]["BidetFunctionality_esMX"];

    response["@search.facets"]["ProductInstallationType"] = response["@search.facets"]["ProductInstallationType_esMX"];
    delete response["@search.facets"]["ProductInstallationType_esMX"];

    response["@search.facets"]["ProductProductType"] = response["@search.facets"]["ProductProductType_esMX"];
    delete response["@search.facets"]["ProductProductType_esMX"];

    response["@search.facets"]["ProductMaterial"] = response["@search.facets"]["ProductMaterial_esMX"];
    delete response["@search.facets"]["ProductMaterial_esMX"];

    response["@search.facets"]["ProductShape"] = response["@search.facets"]["ProductShape_esMX"];
    delete response["@search.facets"]["ProductShape_esMX"];

    response["@search.facets"]["SprayType"] = response["@search.facets"]["SprayType_esMX"];
    delete response["@search.facets"]["SprayType_esMX"];

    response["@search.facets"]["ProductConfiguration"] = response["@search.facets"]["ProductConfiguration_esMX"];
    delete response["@search.facets"]["ProductConfiguration_esMX"];

    response["@search.facets"]["NightLight"] = response["@search.facets"]["NightLight_esMX"];
    delete response["@search.facets"]["NightLight_esMX"];

    response["@search.facets"]["NumberOfHoles"] = response["@search.facets"]["NumberOfHoles_esMX"];
    delete response["@search.facets"]["NumberOfHoles_esMX"];

    response["@search.facets"]["IntelligentToilet"] = response["@search.facets"]["IntelligentToilet_esMX"];
