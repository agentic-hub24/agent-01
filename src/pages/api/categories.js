...
const categories = async (req, res) => {
  authMiddleware(req, res, async () => {
    try {
      ...
      let regionMainCategory =
        'RegionSubCategory' + (lang === 'en' ? '' : '_esMX');
      let regionProductCategory =
        'RegionProductCategoryLocal' + (lang === 'en' ? '' : '_esMX');

      bodyData['facets'] = [`${regionProductCategory},count:100`];

      const categores =
        lang === 'en'
          ? process.env.mainCategores.split(',')
          : process.env.mainCategores_esMX.split(',');
      ...
      const kitchenSubCategories =
        lang === 'en'
          ? process.env.kitchenSubCategories.split(',')
          : process.env.kitchenSubCategories_esMX.split(',');

      const commercialSubCategories =
        lang === 'en'
          ? process.env.commercialSubCategories.split(',')
          : process.env.commercialSubCategories_esMX.split(',');

      const showerSubCategories =
        lang === 'en'
          ? process.env.showerSubCategories.split(',')
          : process.env.showerSubCategories_esMX.split(',');

      if (!categores.includes(category)) {
        res
          .status(405)
          .send({ error: 'Please enter the correct value for category' });
        return;
      }
      ...
      bodyData[
        'filter'
      ] = `ProductIsDiscontinued ne true and SKUDiscontinuedDate eq null and ${regionMainCategory} eq '${category}' and search.in(${regionProductCategory}, '${filterSubCategory.toString()}', ',')`;
